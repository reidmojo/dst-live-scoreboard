...
import { ptwResponses } from "@/db/schema";
...
function routeError(error: unknown) {
  ...
  if (message.includes("no such table") || message.includes("ptw_responses")) {
    return "PTW responses are not available yet. The database migration needs to be applied during deployment.";
  }
  return message;
}
...
export async function GET() {
  try {
    ...
    const rows = await db
      .select()
      .from(ptwResponses)
      .orderBy(desc(ptwResponses.updatedAt), desc(ptwResponses.id));
...
export async function POST(request: Request) {
  try {
    ...
    const [existing] = await db
      .select({ name: ptwResponses.name, selections: ptwResponses.selections })
      .from(ptwResponses)
      .where(eq(ptwResponses.nameKey, nameKey))
      .limit(1);
