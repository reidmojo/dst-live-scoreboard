...
test('kickoff return outside drives credits receiving DST',()=>{const r=score([],null,[{...returnPlay,type:{text:'Kickoff Return Touchdown'},team:{id:'2'}}]);assert.equal(r.dstScores.IND.points,6);assert.equal(r.dstScores.BAL.points,0);});
test('kickoff return drive is not an offensive TD allowed',()=>{const p={...returnPlay,type:{text:'Kickoff Return Touchdown'},team:{id:'2'}};const r=score([drive('TD',{plays:[p]})]);assert.equal(r.dstScores.IND.points,6);assert.equal(r.dstScores.BAL.points,0);});
